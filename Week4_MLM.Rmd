...
*From last week's homework, so code is suppressed.*  
...
*From last week's homework, so output is suppressed.*  
...
```{r Problem4A, warning = FALSE, echo = FALSE}
...
```{r Problem4B, warning = FALSE, results = 'asis'}
#Nominal table
library(reghelper)
library(plyr)
library(dplyr)
...
library(papaja)
papaja::apa_table(tab %>% select(-type), caption = "Nominal Table", na_string = "", 
                  stub_indents = list(Fixed = c(1:4), Random = c(5:8), Model = c(9:11)))

#Continuous table
tab2 <- table_fun(lin.cont2)
papaja::apa_table(tab2 %>% select(-type), caption = "Continuous Table", na_string = "", 
                  stub_indents = list(Fixed = c(1:4), Random = c(5:8), Model = c(9:11)))
```
